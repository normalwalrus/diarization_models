...
def main():
    ...
    #pyannote_model = PYANNOTE_COMMUNITY()
    # pyannote_model = PYANNOTE()
    # reverb_model = REVERB()
    nemo_model = DiarInference()
    #pyannote_deconstructed_model = PYANNOTE_DECONSTRUCTED()

    # rttm_files_done = [f for f in os.listdir('outputs/pyannote_community_1') if f.endswith(".rttm")]
    ...
    for audio_file in LIST_OF_AUDIO_FILES:
        ...
        if basename in nemo_rttm_basename_done_list:
        ...
        # diarization = pyannote_model.diarize_into_rttm(audio_filepath, f'outputs/pyannote_community_1/{basename}.rttm')
